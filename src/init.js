  ...
  async.auto({
    ...
    connect: ['config', 'genesisblock', 'logger', 'network', function (cb, scope) {
      ...
      scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
        scope.logger.log("Belrium started: " + scope.config.address + ":" + scope.config.port);

        if (!err) {
          if (scope.config.ssl.enabled) {
            scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
              scope.logger.log("Belrium https started: " + scope.config.ssl.options.address + ":" + scope.config.ssl.options.port);
