...
Transaction.prototype.verifySignature = function (trs, publicKey, signature) {
  ...
  try {
    ...
    if(trs.type == TransactionTypes.DISABLE_WALLET_KYC) {
      res = true;
    }
  } catch (e) {
    throw Error(e.toString());
...
Transaction.prototype.objectNormalize = function (trs) {
  ...
  if (!report) {
    library.logger.error('Failed to normalize transaction body: ' + this.scope.scheme.getLastError().details[0].message, trs.id);
