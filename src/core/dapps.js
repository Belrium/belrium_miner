...
function OutTransfer() {
  this.create = function (data, trs) {
    console.log("calling out transfer contract");
  ...
  this.calculateFee = function (trs, sender) {
    return constants.fees.outTransfer * constants.fixedPoint;
  }
  ...
  this.apply = function (trs, block, sender, cb) {
    ...
    if (transfer.currency !== 'BEL') {
      ...
      async.series([
        ...
        function (next) {
          library.model.updateAssetBalance('BEL', '-' + trs.fee, transfer.dappId, next)
        },
        function (next) {
      ...
      modules.accounts.setAccountAndGet({ address: trs.recipientId }, function (err, recipient) {
        ...
        modules.accounts.mergeAccountAndGet({
          ...
          library.model.updateAssetBalance('BEL', -amount - trs.fee, transfer.dappId, cb);
        });
      });
    }
  }
  ...
  this.applyUnconfirmed = function (trs, sender, cb) {
    ...
    if (transfer.currency === 'BEL') {
      ...
      var belBalance = library.balanceCache.getAssetBalance(transfer.dappId, 'BEL') || 0
      if (bignum(belBalance).lt(fee)) return setImmediate(cb, 'Insufficient balance')
      if (bignum(balance).lt(transfer.amount)) return setImmediate(cb, 'Insufficient asset balance')
      library.balanceCache.addAssetBalance(transfer.dappId, 'BEL', -fee)
...
function InTransfer() {
  ...
  this.calculateFee = function (trs, sender) {
    return constants.fees.inTransfer * constants.fixedPoint;
  }
  ...
  this.verify = function (trs, sender, cb) {
    ...
    if (asset.currency !== 'BEL') { console.log(true);
  ...
  this.getBytes = function (trs) {
    try {
      ...
      if (trs.asset.inTransfer.currency !== 'BEL') {
  ...
  this.applyUnconfirmed = function (trs, sender, cb) {
    var transfer = trs.asset.inTransfer
    if (transfer.currency === 'BEL') return setImmediate(cb)
...
function DApp() {
  ...
  this.calculateFee = function (trs, sender) {
    return constants.fees.dapp * constants.fixedPoint;
  }
...
private.createBasePathes = function (cb) {
  async.series([
    ...
    function (cb) {
      if(!fs.existsSync(path.join(private.appPath, 'public'))) {
        fs.mkdirSync(path.join(private.appPath, 'public'));
      }
  ...
  private.getInstalledIds(function (err, dappIds) {
    ...
    async.eachSeries(dappIds, function (id, next) {
      ...
      library.model.getDAppByName(WITNESS_CLUB_DAPP_NAME, function (err, clubInfo) {
        if (err) {
          library.logger.error('Failed to query belrium witness club', err)
        } else if (!clubInfo) {
          library.logger.warn('Belrium witness club dapp is not found')
        } else {
