  ...
  library.scheme.validate(query, {
    ...
    library.dbSequence.add(function (cb) {
      ...
      private.getByField(field, function (err, block) {
        ...
        modules.accounts.getAccount({address: block.generatorId}, function(err, account) {
          block.generatorId = block.generatorId + ((account && account.countryCode)? account.countryCode: '');
          cb(null, { block: block });
        });
      });
    }, cb);
...
shared.lastBlock = function(req, cb) {
  var block = private.lastBlock;
  req.body = {publicKey: block.generatorPublicKey};
  modules.delegates.getDelegate(req, function(err, res) {
    if(err) {
      library.logger.error("failed to get last block of generator");
      return cb(err);
    }
    block.generatorName = res.delegate.username;
    ...
    library.dbSequence.add(function (cb) {
      private.list(query, function (err, data) {
        ...
        data.blocks.forEach(function(block, index){
          addresses.push(block.generatorId);
        });
        modules.accounts.getAccounts({
          ...
          rows.forEach(function(row, index1) {
            data.blocks.forEach(function(block, index2) {
              if(row.address == block.generatorId) {
                block.countryCode = row.countryCode;
                block.generatorId = block.generatorId + ((row && row.countryCode)? row.countryCode: '');
              }
            });
          });
