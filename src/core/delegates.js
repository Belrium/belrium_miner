...
Delegates.prototype.getDelegates = function (query, cb) {
  ...
  modules.accounts.getAccounts({
    isDelegate: 1,
    sort: { "vote": -1, "publicKey": 1 }
  }, ["username", "address", "publicKey", "vote", "missedblocks", "producedblocks", "fees", "rewards", "balance", "countryCode"], function (err, delegates) {
    if (err) {
      return cb(err);
    }
    ...
    for (var i = 0; i < delegates.length; i++) {
      ...
      //delete delegates[i].countryCode;
...
Delegates.prototype.getDelegate = function (req, cb) {
  var query = req.body;
  library.scheme.validate(query, {
    type: "object",
    properties: {
  ...
  library.scheme.validate(query, {
    ...
    library.dbLite.query("select GROUP_CONCAT(accountId) from mem_accounts2delegates where dependentId = $publicKey", {
      ...
      modules.accounts.getAccounts({
        ...
        sort: 'balance'
      }, ['address', 'balance', 'publicKey', 'username', "countryCode"], function (err, rows) {
        if (err) {
          library.logger.error(err);
          return cb("Database error");
        }
        ...
        rows.forEach(function (row) {
          ...
          row.address = row.address + ((row && row.countryCode)? row.countryCode: '');
        });
